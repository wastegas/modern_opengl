  ...
  while(!glfwWindowShouldClose(window))
    {
      ...
      // stencil buffer, we only care about the containers.
      // we will set its mask to 0x00 to not
      ...
      // 1st pass draw objects as normal, writing to the stencil buffer
      ...
      // 2nd pass, now draw slightly scalled of version of the objects,
      // this time disabling stencil writing
      // becase the stencil buffer is now filled with several 1s. the parts
      // of the buffer that are 1 are not drawn,
      // thus only drawing the objects size differences making it look
      // like borders
      glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
      glStencilMask(0x00);
